...
## display : 목록 내 항목의 위치 변경
요소의 동작을 변경
블록-> 인라인,인라인 블록으로 두 속성 모두를 갖게하거나, 돔에서 완전히 삭제도 가능

html에는 인라인 요소와 블록 레벨 요소가 존재

인라인 요소로는 a태글 들 수 있음, 한 줄에서 렌더링 되기 때문

![alt text](image-12.png)

![alt text](image-13.png)

두 줄이 같은 줄에 렌더링되며 필요한 만큼만 차지, 블록 레벨처럼 전체 너비를 사용하지 않음

여기서는 li 태그가 블록레벨

인라인은 마진 설정이 안됨, 설정이 불가능 -> 마진을 두면 다른 요소와 줄이 겹칠 우려가 있음
...
두번째는 block이 되어 한 줄을 차지


![alt text](image-16.png)

display를 none으로 지정 가능한데 이러면 안보이게 됨, 이것은 완전히 삭제된 것이 아니라 표시되는 문서 대열에서만 빠진 것

display:none이 쓰이는 경우
- JavaScript와 같이 쓰이는 경우
- 가령 버튼이 클릭된 후 동작에서 활용
- 특정 동작이 이루어졌을 때만 표시되도록 모달을 생성하는데 활용
